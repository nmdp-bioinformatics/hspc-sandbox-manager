...
cat container-definitions_prod.json | jq --arg container_port $PROJECT_PORT '.[0].name=$container_port' 2>&1 | tee container-definitions_prod.json
...
cat container-definitions_test.json | jq --arg container_port $PROJECT_PORT '.[0].name=$container_port' 2>&1 | tee container-definitions_test.json
