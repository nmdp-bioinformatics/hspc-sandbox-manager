...
//                    $rootScope.$digest();
    ...
    return {
        ...
        addLaunchScenario: function(launchScenario){
            ...
            appsSettings.getSettings().then(function(settings){
                $.ajax({
                    url: window.location.href.split("#")[0].substring(0, window.location.href.split("#")[0].length-1) + "/launchScenario",
                    type: 'POST',
        ...
        getLaunchScenarios: function() {
            ...
            appsSettings.getSettings().then(function(settings){
                $.ajax({
                    url: window.location.href.split("#")[0].substring(0, window.location.href.split("#")[0].length-1) + "/launchScenarios?id=" + encodeURIComponent(userServices.oauthUser().ldapId),
                    type: 'GET',
                    contentType: "application/json"
